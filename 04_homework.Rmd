...
just by looking on the plot, we may suppose that the data might be well divided according to the trasmission method (column x11), infact the observation belonging to $x11 = 0$ are in the x1 range $[100, 200]$. While, the others belong to the range $[200, 500]$. Even if the dataset is splitted in two subset, the kind of relationship between y and x1 is still visible, but this time we may suppose a relation for the group $x1 = 0$  than the other group.
...
The point 5 seems to be both an outlier and an influencer because its cook distance is very large, moreover its residual is significaly far from zero, in the end it's the only pointed fittend in the first portion of the model.
